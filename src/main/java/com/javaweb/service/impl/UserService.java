...
public class UserService implements IUserService {
    ...
    public Map<Long, String> getListStaff() {
        ...
        staffs.forEach(item -> {
            staffList.put(item.getId(),item.getFullName());
        });
        return staffList;
    }
    ...
    @Override
    @Transactional
    public UserDTO insert(UserDTO newUser) throws MyException {
        if(userRepository.findOneByUserName(newUser.getUserName()) != null) {
            throw new MyException("Username already exists, please choose another username!");
        }
